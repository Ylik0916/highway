<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="85px">
      <el-form-item label="建制村名称" prop="nameOfVillage">
        <el-input
          v-model="queryParams.nameOfVillage"
          placeholder="请输入建制村名称"
          clearable
...
<!--        />-->
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:village:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:village:export']"
        >导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            @click="getVillageXq(scope.row)"
            v-hasPermi="['system:village:edit']"
          >详情</el-button>
          <el-button
            size="mini"
            type="text"
    ...
    <el-dialog :title="title" :visible.sync="openXq" width="1000px" append-to-body>
      <el-tabs v-model="activeName" type="card">
        <el-tab-pane label="建制村信息" name="first">
          <el-descriptions :model="form">
            <el-descriptions-item label="建制村名称">{{ form.nameOfVillage }}</el-descriptions-item>
            ...
            <el-descriptions-item label="行政区划">{{ form.administrativeDivisionNumber }}</el-descriptions-item>
            ...
            <el-descriptions-item label="码头与陆地距离">{{ form.distanceFromPierToLand }}</el-descriptions-item>
            ...
            <el-descriptions-item label="优选通达路线行政等级">{{ form.administrativeLevelAccessRoute }}</el-descriptions-item>
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" style="display: flex;flex-wrap: wrap;justify-content: space-between">
        <el-form-item label="建制村名称" prop="nameOfVillage">
          <el-input v-model="form.nameOfVillage" placeholder="请输入建制村名称" />
        </el-form-item>
        ...
        <el-form-item label="行政区划" prop="administrativeDivisionNumber">
          <el-input v-model="form.administrativeDivisionNumber" placeholder="请输入行政区划" />
        </el-form-item>
        ...
        <el-form-item label="码头与陆地距离" prop="distanceFromPierToLand">
          <el-input v-model="form.distanceFromPierToLand" placeholder="请输入码头与陆地距离" />
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        nameOfVillage: [
          { required: true, message: "建制村名称不能为空", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const villageIds = row.villageId || this.ids;
      this.$modal.confirm('是否确认删除建制村编号为"' + villageIds + '"的数据项？').then(function() {
        return delVillage(villageIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/village/export', {
        ...this.queryParams
